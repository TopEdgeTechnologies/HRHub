...
namespace HRHUBWEB.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> HR()
        {
            ...
            ViewBag.ComponentInfoStatus = await _APIHelper.CallApiAsyncGet<IEnumerable<ComponentInfo>>($"api/StaffBenefits/GetComponentsInfos{_user.CompanyId}", HttpMethod.Get);
            ViewBag.LeaveTypeStatus = await _APIHelper.CallApiAsyncGet<IEnumerable<LeaveType>>($"api/Configuration/GetLeaveTypeInfos{_user.CompanyId}", HttpMethod.Get);
            ViewBag.OffBoardingStatus = await _APIHelper.CallApiAsyncGet<IEnumerable<StaffOffBoarding>>($"api/Setting/GetOffBoardingSetting{_user.CompanyId}", HttpMethod.Get);
            ViewBag.CompanyInfo = await _APIHelper.CallApiAsyncGet<Company>($"api/Company/GetCompanyInfoId{_user.CompanyId}", HttpMethod.Get);
        ...
        public async Task<IActionResult> StaffMonthlyAttendance(int currentDate)
        {
            ...
            object[] parameters = new object[] { _user.CompanyId ?? 0, currentDate, _user.StaffId };
        ...
        public async Task<IActionResult> Staff_Attendance_Present_Leave()
        {
            ...
            object[] parameters = new object[] { _user.CompanyId ?? 0, _user.StaffId };
        ...
        [HttpGet]
        public async Task<IActionResult> Staff_Attendance_Calendar()
        {
            int year = DateTime.Now.Year;
            DateTime startDate = new DateTime(year, 1, 1);
            DateTime endDate = DateTime.Now;

            string procrdure = "BI.GetStaff_Attendance_Calendar";
            object[] parameters = new object[] { _user.CompanyId ?? 0, "'" + startDate.ToString("dd-MMM-yyyy") + "'", "'" + endDate.ToString("dd-MMM-yyyy") + "'", _user.StaffId };
